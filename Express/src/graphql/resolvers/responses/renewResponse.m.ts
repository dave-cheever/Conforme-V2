...
const renewResponse = async (_, { _id }, { authorize, organization }) => {
  try {
    ...
    const createSnapshot = async () => {
      ...
      const users = await GraphService.getBasicUsers({
        usersIds,
        organization,
      });
            ...
            response: {
              old: {
                value: {
                  ...
                  accountable: users.find(
                    ({ _id }) => _id === response.accountableId,
                  ),
                  contributors: users.filter(({ _id }) =>
                    response.contributorsIds?.includes(_id),
                  ),
                  followers: users.filter(({ _id }) =>
        ...
        .map(({ type, name, description, required, value, requiredAnswer }) => {
          if (type === 'multipleChoice') {
            return {
              ...
              description,
              required,
              value: (value as IQuestionChoice[])?.map((choice) => ({
                ...choice,
                isCorrect: false,
              })),
            };
