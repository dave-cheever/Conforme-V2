...
const deleteBusinessUnit = async (_, { _id }, { authorize, organization }) => {
  try {
    ...
    const businessUnit = await BusinessUnits.customFindById(_id, organization._id);
    if (!businessUnit) {
      throw new Error("Business Unit doesn't exist");
    }

    const deletedResult = await BusinessUnits.customDelete({ _id }, user._id, organization._id);
