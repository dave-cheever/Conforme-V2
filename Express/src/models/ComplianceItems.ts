...
complianceItemSchema.statics.customCreate = async function (
...
complianceItemSchema.statics.customFind = async function (
  selector: any = {},
): Promise<IComplianceItem[]> {
...
complianceItemSchema.statics.customFindOne = async function (
  selector: any = {},
  organizationId: string,
): Promise<IComplianceItem | null> {
...
complianceItemSchema.statics.customFindById = async function (
  _id: string,
): Promise<IComplianceItem> {
  const complianceItem = await this.findOne({
    _id,
  ...
  if (!complianceItem) throw new Error('Compliance item not found');

  return complianceItem;
};

complianceItemSchema.statics.customUpdateOne = async function (
  selector: object = {},
  updates: Partial<IComplianceItem>,
  userId: string,
  organizationId: string,
): Promise<IComplianceItem> {
  const complianceItem = await this.customFindOne(selector, organizationId);
  if (!complianceItem) throw new GraphQLError("Compliance item doesn't exist");
...
complianceItemSchema.statics.customDelete = async function (
  selector: object = {},
  userId: string,
  organizationId: string,
): Promise<number> {
  const complianceItem = await this.customFindOne(selector, organizationId);
  if (!complianceItem) throw new GraphQLError("Category doesn't exist");
...
complianceItemSchema.statics.customGenerateReference =
  async function (): Promise<string> {
    let reference = '0000001';
    const lastComplianceItem = await this.findOne({})
      .sort({ 'metatags.addedAt': -1 })
      .lean();
    if (lastComplianceItem) {
      const newReference = parseInt(lastComplianceItem.reference, 10) + 1;
