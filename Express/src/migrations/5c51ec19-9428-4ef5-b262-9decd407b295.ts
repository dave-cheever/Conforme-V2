...
* Excel file link: https://cielocosta.sharepoint.com/:x:/r/teams/DocumentControls-BRE/Shared%20Documents/General/BRE%20%20Group%20%20Document%20Register%20May%202022.xlsx?d=wdf13a109e0d84584b3a460155f524c06&csf=1&web=1
* Parsing is done according to given sample data provided by BRE
* It imports Documents to the system from the BRE Group Documents Register spreadsheet
      ...
      try {
        ...
        if (rowIndex !== -1) {
          ...
          for (const [index, data] of enumerate(filteredJSONData)) {
            try {
              if (isEmpty(data) || (!('Title/Description' in data) && !('Owner' in data) && !('Business/Centre' in data))) continue
              await log(`\n\n\tParsing row ${rowIndex + index + 2}`);

              // if 'Owner', 'Title/Description', 'Business/Centre' are not in the sheet it will not insert them as it is required to create one
              ...
              if (!('Business/Centre' in data) || typeof data['Business/Centre'] !== 'string') throw new Error(`Document does not contain "Business/Centre" header`);
...
export default createBREGroupDocuments;
