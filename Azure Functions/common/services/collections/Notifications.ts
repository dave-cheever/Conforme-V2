...
notificationsSchema.statics.customCreate = async function (notification: INotification, userId: string, organizationId: string): Promise<INotification> {
...
notificationsSchema.statics.customFind = async function (selector: any = {}, organizationId: string): Promise<INotification[]> {
...
notificationsSchema.statics.customFindOne = async function (selector: any = {}, organizationId: string): Promise<INotification | null> {
...
notificationsSchema.statics.customFindById = async function (_id: string): Promise<INotification> {
  const notification = await this.findOne({
    _id,
    "metatags.removedAt": { $eq: null },
  }).lean();
...
notificationsSchema.statics.customUpdateOne = async function (selector: object = {}, updates: Partial<INotification>, userId: string, organizationId: string): Promise<INotification> {
...
notificationsSchema.statics.customDelete = async function (selector: object = {}, userId: string, organizationId: string): Promise<number> {
  const notification = await this.customFindOne(selector, organizationId);
  if (!notification) {
    throw new Error('Notification doesn\'t exist');
  }
...
const notificationModel = model<INotification, INotificationModel>("Notification", notificationsSchema);
