...
const Audits = () => {
  ...
  const { filtersValues, setFilters, setResponsesStatusesCounts, setShowFiltersPanel } = useFiltersContext();
  const [filteredResponses, setFilteredResponses] = useState<IResponse[]>([]);
  const { getRenewalStatus, getStatus } = useResponseUtils();

  const { data, loading, error, refetch } = useQuery(GET_RESPONSES, { skip: true });
            ...
            {viewMode === "Grid" &&
              <Grid templateColumns={["repeat(1, 1fr)", "repeat(2, 1fr)", ""]} display={["grid", "grid", "flex"]} flexWrap='wrap' h="fit-content" gap={6} w='full' pb={[0, 8]} px={[4, 8]} pt="3">
                {filteredResponses.length > 0
                  ...
                  : <Flex w='full' h='full' fontSize='18px' fontStyle='italic'>No audits found</Flex>
                }
              </Grid>}
