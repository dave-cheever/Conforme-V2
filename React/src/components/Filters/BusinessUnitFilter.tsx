...
const BusinessUnitFilter = () => {
  ...
  const { filtersValues, setFilters, businessUnits, areas } =
    useFiltersContext();
  const value = useMemo(
    () =>
      module?.type === 'tracker'
        ? filtersValues.businessUnitsIds?.value
        : filtersValues.areasIds?.value,
  ...
  return (
    <Box w="full">
      <BusinessUnitsSelector
        businessUnits={
          module?.type === 'tracker'
            ? (businessUnits as IBusinessUnit[])
            : (areas as IBusinessUnit[])
        }
